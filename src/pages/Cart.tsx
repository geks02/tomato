...
const Cart = () => {
  ...
  const increasedQty = async (itemId: string) => {
    try {
  ...
  const decreasedQty = async (itemId: string) => {
    try {
      setLoadingItemId(itemId);
      await axios.put(
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6 space-y-6">
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        <button
          onClick={checkOut}
          className={`mt-3 w-full rounded-lg bg-[#E23744] py-3 text-sm font-semibold text-white
        hover:bg-red-800 ${!restaurant.isOpen ? "opacity-50 cursor-not-allowed" : ""}
        `}
        ...
        <button
          onClick={clearCart}
          className="mt-3 w-full rounded-lg bg-[#232222] py-3 text-sm font-semibold text-white
        hover:bg-gray-900 flex justify-center items-center gap-3
        "
          disabled={clearingCart}
        >
          Clear Cart{" "}
          <BiTrash size={16} className="items-center justify-center " />
        </button>
      </div>
    </div>
  );
