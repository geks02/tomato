...
const RiderDashboard = () => {
  ...
  useEffect(() => {
    // ✅ Fix: Fetch profile only if the user IS a rider
    if (user?.role === "rider") {
      fetchProfile();
    } else {
      setLoading(false);
  ...
  return (
    <div className="space-y-4">
      ...
      {!audioUnlocked && (
        <div
          className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex 
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 ">
                Get Notified when new orders arrived
              </p>
            </div>
          </div>
          <button
      ...
      {/* {profile.isAvailable && incomingOrders.length > 0 && (
        <div className="mx-auto max-w-md px-2 space-y-3">
          ...
          {currentOrder && (
            <div className="mx-auto max-w-md px0-4 space-y-4">
              <RiderCurrentOrder
                order={currentOrder}
      ...
      {/* FIX: INCOMING ORDERS SECTION (Independent) */}
      {profile.isAvailable && incomingOrders.length > 0 && (
        <div className="mx-auto max-w-md px-4 space-y-3">
          <h3 className="font-semibold text-gray-700">Incoming Requests</h3>
          {incomingOrders.map((id) => (
            <RiderOrderRequest
              key={id}
              orderId={id}
              onAccepted={() => {
                // Remove the order from local incoming list immediately
                setIncomingOrders((prev) => prev.filter((item) => item !== id));
                fetchProfile();
                fetchCurrentOrder();
              }}
            />
      ...
      {/* FIX: CURRENT ORDER SECTION (Moved outside incomingOrders check) */}
      {currentOrder && (
        <div className="mx-auto max-w-md px-4 space-y-4">
          <h3 className="font-semibold text-gray-700">Active Delivery</h3>
          <RiderCurrentOrder
            order={currentOrder}
            onStatusUpdate={fetchCurrentOrder}
          />
