  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
      <div className="flex justify-between items-center">
        ...
        <span
          className={`rounded-full px-3 py-2 text-xs font-medium ${statusColor(order.status)} `}
        >
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          {actions.map((status) => (
            <button
              ...
              className="rounded-lg bg-[#E23744] px-3 py-1 text-xs text-white
            hover:bg-[#D32F3A] disabled:opacity-50"
            >
      ...
      {order.status === "ready_for_rider" && retryVisible && (
        <div className="pt-2">
          <button
            className="w-full rounded-lg border border-[#E23744] py-2 text-xs font 
